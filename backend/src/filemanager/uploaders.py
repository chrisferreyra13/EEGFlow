...
from filemanager.models import TemporaryUpload, storage,\
    TemporaryUploadChunked
from io import BytesIO, StringIO
from filemanager.utils import _get_user
...
#----UPLOADERS----#
class FileManagerFileUploader(object):
    ...
    def get_uploader(cls, request):
        ...
        if request.method == 'PATCH':
            return FileManagerChunkedFileUploader()
        ...
        return FileManagerStandardFileUploader()

    @classmethod
    def _get_file_obj(cls, request):
        # By default the upload element name is expected to be "filemanager"
        # As raised in issue #4, there are cases where there may be more
        # than one filemanager instance on a page, or the developer has opted
        # not to use the name "filemanager" for the filemanager instance.
        # Using the example from #4, this provides support these cases.
        upload_field_name = 'filemanager'
        if 'fm_upload_field' in request.data:
            upload_field_name = request.data['fm_upload_field']
        ...
        # request.data['filemanager'], if there is more than one value provided
    ...
    # filemanager.views. The TemporaryUpload model should validate
...
class FileManagerStandardFileUploader(FileManagerFileUploader):
    ...
    def handle_upload(self, request, upload_id, file_id):
        ...
        #    LOG.debug('FileManager app: Creating file upload directory '
        #             '<%s>...' % storage.location)
...
class FileManagerChunkedFileUploader(FileManagerFileUploader):
