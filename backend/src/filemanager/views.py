...
from filemanager.renderers import PlainTextRenderer
...
from filemanager.uploaders import FileManagerFileUploader
...
class ProcessView(APIView):
    '''
    This view receives an uploaded file from the client. It
    ...
    def post(self, request):
        LOG.debug('FileManager API: Process view POST called...')
...
class PatchView(APIView):
    ...
    def patch(self, request, chunk_id):
        LOG.debug('FileManager API: Patch view PATCH called...')
        uploader = FileManagerFileUploader.get_uploader(request)
    ...
    def head(self, request, chunk_id):
        LOG.debug('FileManager API: Patch view HEAD called...')
        uploader = FileManagerFileUploader.get_uploader(request)
        return uploader.handle_upload(request, chunk_id)
...
class LoadView(APIView):
    ...
    directory specified by the FILEMANAGER_FILE_STORE_PATH
    setting parameter).
    """
    permission_classes = _import_permission_classes('GET_LOAD')

    def get(self, request):
        LOG.debug('FileManager API: Load view GET called...')
...
class RestoreView(APIView):
    ...
    def get(self, request):
        LOG.debug('FileManager API: Restore view GET called...')
        if LOAD_RESTORE_PARAM_NAME not in request.GET:
            return Response('A required parameter is missing.',
                            status=status.HTTP_400_BAD_REQUEST)
...
class FetchView(APIView):
    ...
    def _process_request(self, request):
        LOG.debug('FileManager API: Fetch view GET called...')
        '''
        Supports retrieving a file on the server side that the user has
        specified by calling addFile on the FileManager API and passing a
        URL to a file.
        '''
    ...
    def head(self, request):
        LOG.debug('FileManager API: Fetch view HEAD called...')
